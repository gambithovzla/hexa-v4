...
- **11.5** 🟡 player props — fundación (parser + resolver de boxscore ESPN) en [soccer-props-resolver.js](server/soccer-props-resolver.js); pendiente in-season: odds/board/modelo/UI. **11.6** ✅ parlay (`soccerParlayCandidates.js` → motor frozen; `POST /api/soccer/parlay`, flag `PARLAY_SYNERGY_SOCCER_ENABLED`). **11.7** Imperdible Soccer. **11.8** CLV + postmortem + ensemble. **11.9** smart signals + ascensos/descensos.
