...
### ✅ Sprint 8g — MLB effectiveness layer (Q3 2026)

**Status**: ✅ **cerrado** (2026-06-09) — rama `claude/statcast-player-data-rnlsxa`.

**Problema que resolvía**: el pipeline MLB pregame era profundo (8d) pero (1) no aprendía de sus errores (postmortems write-only), (2) la confianza del Oracle no estaba calibrada contra hit rates reales, (3) el CLV se capturaba y nadie lo leía, (4) line movement no distinguía steam de ruido, (5) el acuerdo Oracle/validador/sidecar era invisible al usuario, (6) un lineup sin publicar dejaba el Statcast individual vacío en silencio (y el OR de `lineupStatus` marcaba "confirmado" con un solo equipo publicado).

**Entregables**:

| Item | Archivos | Qué hace |
|---|---|---|
| Lineup `partial` status | `mlb-api.js`, `context-builder.js`, `AnalysisPanel.jsx`, `GameSelector.jsx` | `confirmed` exige AMBOS lineups; un solo lineup = `partial`; brief nombra al equipo faltante; chip por juego en GameSelector (`✓ LINEUPS` / `½ FALTA <ABBR>` / `SIN LINEUPS`); data quality puntúa batter Statcast por lado (5+5) |
| Lineup proyectado fallback | `mlb-api.js` (`getTeamProjectedLineup`), `context-builder.js` | Sin lineup publicado → batting order del último juego completado (boxscore); Statcast/splits/H2H individuales fluyen con etiqueta `⚠️ PROJECTED LINEUP` |
| Lessons Learned | `services/postmortemLessonsService.js` (NEW) | Agrega `adjustment_signals` de postmortems (30d, frequency-ranked, cache 1h) → bloque `ORACLE LESSONS LEARNED` en el contexto. Cierra el feedback loop |
| Calibración de confianza | `services/confidenceCalibrationService.js` (NEW), `picks.calibrated_confidence` | Shrinkage de la confianza Oracle hacia el win rate observado por mercado+bucket (≥15 picks resueltos; a 200+ domina lo observado). Fire-and-forget post-save |
| CLV report | `routes/admin-ml.js` (`GET /api/admin/ml/clv-report`), panel en `AdminMLControlCenter.jsx` | CLV promedio + win rate por mercado y bucket de confianza (90d). CLV positivo sostenido = edge real; negativo sistemático = matar/degradar ese mercado |
| Conviction tier | `services/convictionService.js` (NEW), `picks.conviction_tier`, badge en `HistoryPanel.jsx` | Tier `3/3`/`2/3`/… = modelos de acuerdo con el pick (Oracle + shadow validator + sidecar Python). Sweep job cada 30 min desde `shadow_model_runs` |
| Line movement v2 | `line-movement.js`, `odds-api.js` (`perBook` aditivo en `normalizeEvent`), `migrate.js` | Snapshots con `bookmaker_count` + precios per-book; `sustained_move_pct` (steam vs ruido book-a-book); `reverse_line_movement` (favorito alargándose = sharps contra el público); render en bloque LINE MOVEMENT |
| Starts trend | `mlb-api.js` (`getPitcherStartsTrend`), `context-builder.js` | Últimas 5 salidas del starter (IP/K/BB/ER + ERA agregada) como bloque `PITCHER RECENT STARTS TREND`; trayectoria > ERA de temporada |

**Bug latente arreglado (significativo)**: `contextString` era `const` en `buildContext`, así que el `contextString += ragBlock` existente lanzaba un TypeError silencioso dentro de su try/catch — **el bloque RAG `SIMILAR PAST ANALYSES` nunca llegó al Oracle en producción**. Fix: `let` + resync de `result.context`, lo que activa RAG + Lessons.

**Pendiente operacional**: lessons/calibración/CLV/conviction necesitan acumulación de datos (postmortems generados, ≥15 picks resueltos por bucket, CLV capturado, shadow runs); no requieren más código.
...
|---|---:|---:|---:|---|
| Data depth pregame | 9.5 | 8.5 🟢 | 8.0 | 11.3 ✅ |
| Lineup/Injury/Suspensión | 9.0 | 8.5 🟢 | 8.0 | 11.3 ✅ (API-Football) |
| Market coverage real | 9.0 | 7.5 🟡 | 8.0 | 11.5 ✅ (props board + UI) |
...
#### Sprint 11.3 — Profundidad de contexto pregame (el "Statcast del fútbol") — ✅ implementado completo (2026-06-08)
...
- ✅ **Motivación/stakes** — `LEAGUE_SPOTS` (constante nueva: zonas UCL/UEL/UECL, descenso, playoff por liga); `extractFullStandings` (tabla completa de posiciones desde el payload ESPN ya fetcheado); `buildMotivationBlock()` (puro, exportado): categoriza TITLE LEADERS / TITLE RACE / UCL|UEL|UECL PLACE / CONTENDER / RELEGATION ZONE / RELEGATION BATTLE / PLAYOFF ZONE (MLS) / MID-TABLE. Threshold contender: ≤3pts de la frontera. `extractTeamFromStandings` ahora retorna `position` y `gamesPlayed`. `buildTeamBlock` incluye `motivation: null`; `buildSoccerGameContext` lo computa post-standings sin nueva llamada API. `oracleSoccer.describeMotivationLine()` renderiza la línea Stakes en cada bloque de equipo. Prompt: señal #11 STAKES/MOTIVATION con modificadores por categoría + 2 alert flags (six-pointer, dead rubber). Tests: `soccerMotivation.test.js` (+20 casos: EPL/Bundesliga/Ligue 1/MLS + thresholds + edge cases).
- ✅ **Árbitro con tendencias** (2026-06-08) — `normalizeRefereeStats` (export puro) + `_fetchRefereeStats` en [soccer-lineups-api.js](../server/soccer-lineups-api.js): API-Football `/referees`, cache 24h; `yellowsPerGame`, `redsPerGame` (roja directa + 2.ª amarilla), `penaltiesPerGame`. 8.ª promesa en `getSoccerMatchAvailability`; `refereeStats` en context + `sources.refereeStats` + staleFlag `referee_stats_unavailable`. `oracleSoccer` renderiza labels `lenient/average/card-heavy`; prompt señal #10 condicional (con stats: ±2% max; sin stats: solo nombre). Tests: [soccerRefereeStats.test.js](../server/__tests__/soccerRefereeStats.test.js) (10).
- ✅ **xG rolling + PPDA + xGDiff** (2026-06-08) — `computeXgStats` (export puro en [soccer-xg-fetcher.js](../server/soccer-xg-fetcher.js), sin fetch mocking): ventanas `xG_7`/`xG_5` (media últimos 7/5 partidos), `ppda`/`ppdaAllowed` (Passes Per Defensive Action = presión media por partido), `xGDiff`/`xGADiff` (over/underperformance acumulada — positivo=encima de expectativa). Prompt señal #2 extendida: PPDA <8 elite, 8-10 high, 10-13 moderate, >17 passive; xGDiff ≥3 significant. Tests: [soccerXgRolling.test.js](../server/__tests__/soccerXgRolling.test.js) (12, pure function).
- ✅ **FBref set-piece SCA/GCA** (2026-06-08) — [soccer-fbref-fetcher.js](../server/soccer-fbref-fetcher.js) — scraper HTML sin deps nuevas: `parseFBrefSquadTable` (export puro, regex `data-stat`, múltiples table IDs fallback, normaliza nombres de clubs) + `getSoccerGameSetPieceStats(leagueSlug, home, away)` → `{scaDeadBallPer90, gcaDeadBallPer90, scaDeadBallAllowedPer90, gcaDeadBallAllowedPer90}`. League mapping EPL/La Liga/Serie A/Bundesliga/Ligue 1/MLS → FBref comp IDs. Cache 6h, null graceful. Cableado en context builder (5.ª promesa): `home.setpiece`/`away.setpiece` + `sources.setpiece` + staleFlag `setpiece_unavailable`. `oracleSoccer` `describeSetPieceLine`: tag `★ high set-piece threat` (SCA ≥2.0/90) y `★ vulnerable to set pieces` (SCA against ≥2.0/90). Prompt señal #2b SET PIECES — mismatch SCA/GCA dead-ball/90 como edge táctico. Tests: [soccerFbref.test.js](../server/__tests__/soccerFbref.test.js) (7).
- **Criterio de éxito**: ✅ **alcanzado**. Context soccer con bloques LINEUP/INJURIES, VENUE/WEATHER, HEAD-TO-HEAD + REFEREE + REFEREE STATS, SCHEDULE/CONGESTION, HOME/AWAY SPLIT, STAKES/MOTIVATION, xG rolling/PPDA/xGDiff, SET-PIECE SCA/GCA; `context_meta.completeness` rebalanceada; data depth 8.5 🟢. **Sprint 11.3 cerrado** — 49 tests totales (20 motivation + 12 xG/PPDA + 10 referee + 7 fbref).
...
- **Pendiente**: Understat histórico para cablear xG también al frame de pre-training (11.2 lo deja NaN). FBref set-piece/xG ya implementado en 11.3.

#### Sprint 11.5 — Player Props Soccer (espeja NFL 9f–9i) — ✅ board + UI implementados (2026-06-08)
...
- ✅ **board + UI (2026-06-08)**: [soccer-props-odds.js](../server/soccer-props-odds.js) — Odds API event endpoint league-aware (`player_goal_scorer_anytime`→`anytime_goal`, `player_shots_on_target`, `player_shots`, `player_to_receive_a_card`→`card`); dual-key + 5min cache; `normalizeSoccerPropEvent` puro. [soccerPropFeatureEnricher.js](../server/services/soccerPropFeatureEnricher.js) — de-vig de pares over/under → `fairProb`+`vig`; yes-markets sin fairProb. `GET /api/soccer/props/board` (admin, `SOCCER_PROPS_ENABLED`, flag) en [routes/soccer.js](../server/routes/soccer.js): todas las 6 ligas o filtrado por `league`; `oraclePropPicks` desde picks table; `modelProb: null` hasta training in-season. [PlayerPropsPage.jsx](../client/src/pages/PlayerPropsPage.jsx) extendida: ⚽ soccer como 3er deporte (grass-green), columna Fair %, league selector; `/props?sport=soccer`. Tests: `soccerPropsOdds.test.js` (11: `normalizeSoccerPropEvent` + `enrichSoccerPropOffers`).
- **Pendiente (in-season)**: modelo `soccer_prop` pooled pick-aligned (≥50 picks resueltos + `enrichAndPersistSoccerPropPick`). **Nota**: forma exacta del boxscore ESPN soccer validable en temporada.
...
- ✅ **UI dedicada (2026-06-08)**: [ImperdibleSoccerPage.jsx](../client/src/pages/ImperdibleSoccerPage.jsx) — espeja `ImperdiblePage.jsx` pero soccer-native: league selector (6 ligas), endpoints `/api/soccer/imperdible/*`, acento grass-green, umbral 60% (cap 62%), `model/mercado/shadow` en vez de `model/mercado/ML`, `REASON_TEXT` soccer. Ruta `/admin/imperdible-soccer` en `App.jsx`.
- **Pendiente**: calibración de thresholds (4-6 semanas in-season) + gate de lineup real (11.3/API-Football habilitado en producción).
...
2026 Q3  Sprint 8g     — MLB effectiveness layer          ████████████████████████ ✅ (lessons+calibración+CLV+conviction+LM v2; 2026-06-09)
...
- ✅ **Sprint 8g — MLB effectiveness layer (2026-06-09)**: lineup `partial` + chip por juego + lineup proyectado fallback; bloque `ORACLE LESSONS LEARNED` (postmortems agregados); `picks.calibrated_confidence` (shrinkage por bucket real); reporte CLV admin por mercado+bucket; `picks.conviction_tier` (acuerdo Oracle+validador+sidecar, badge en historial); line movement v2 (per-book, steam `sustained_move_pct`, reverse line movement); `PITCHER RECENT STARTS TREND`. **Fix latente**: el bloque RAG `SIMILAR PAST ANALYSES` nunca llegaba al Oracle (`contextString` era const, TypeError silencioso) — activado.
...
- Sprint 8g: lessons/calibración/CLV/conviction necesitan acumulación de datos (postmortems generados, ≥15 picks resueltos por bucket de confianza, CLV capturado, shadow runs recientes) — sin código pendiente.
