...
### NHL (🟡 Sprint 10 — backend completo, UI pendiente)
Cuarto deporte. Espeja **exactamente** el patrón NBA (date-based) que espeja MLB: archivos `nhl-*` nuevos que importan los frozen, `sport='nhl'` en toda persistencia, cero ediciones a frozen. **Backend 10a–10c + shadow/dataset mergeados**; flag `NHL_ANALYSIS_ENABLED` (default `false`). UI (10d) e ML sidecar (10e) **pendientes** — el selector NHL sigue inactivo en `client/src/config/sports.js` para no romper GameSelector/AnalysisPanel. Roadmap: [docs/roadmap.md](docs/roadmap.md) Sprint 10.
...
- **Ya en código**:
  ...
  - [client/src/utils/nhlLogoUrl.js](client/src/utils/nhlLogoUrl.js) — logo ESPN por abbr (listo para 10d).
  - `chatPickExtractor.js` extendido a `'nhl'` (puck line parsea por el path spread; persiste `sport='nhl'` sin contaminar mlb).
- **Pendiente**: **10d UI** (`GameSelector`/`AnalysisPanel`/`SportSwitcher` + activar en `sports.js`), **10e ML sidecar**, `hexaNhlBoardService`, live tracker, validación E2E en prod con `NHL_ANALYSIS_ENABLED=true`.
