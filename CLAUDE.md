...
**NFL — ✅ Sprint 9 + 9.3 + props/board/parlay (9f–9j) completos (2026-06-07)**: tercer deporte operativo. Spec en [docs/nfl-architecture.md](docs/nfl-architecture.md), roadmap en [docs/nfl-roadmap.md](docs/nfl-roadmap.md). Todo el código está mergeado: Oracle, lifecycle, shadow/dataset, live tracker, UI, ML scaffolding + nflverse pre-training (9a–9e + 9.3), **player props** (9f fundación+board → 9g modelo `nfl_prop` pooled → 9h enriquecimiento player-level nflverse + persistencia `source='live'`), **pizarra `hexaNflBoardService` + UI de props parametrizada por sport** (9i), y **Parlay Synergy NFL MVP** vía el motor frozen (9j, flag `PARLAY_SYNERGY_NFL_ENABLED`). **Sprint 9.3** dejó EPA/success/PROE como datos reales y los 3 modelos de juego (`nfl_moneyline`/`nfl_spread`/`nfl_total`) vivos en producción. Pendiente: **Imperdible NFL** (diferido — el más acoplado a MLB, necesita gate QB + data de temporada); entrenamiento real del modelo `nfl_prop` + enriquecimiento in-season de model probs en el parlay + validación E2E requieren temporada (sept 2026, sin data viva en off-season).
