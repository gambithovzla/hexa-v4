...
- **Parity con MLB (planificado, Sprint 11.2–11.9 — ver [docs/roadmap.md](docs/roadmap.md))**: Soccer pasó el MVP pero le faltan los pilares de profundidad que hicieron robusto a MLB (Sprint 8d context enrichment + props + ensemble + pretraining). Brechas rojas vs release-gate: **data depth pregame (4.5)**, **lineups/injuries/suspensiones (2.0)**, **market coverage/props (5.5)**, **calibración ML (3.0)**. Mismo patrón espejo de NFL 9.x, cero ediciones a frozen:
  ...
  - **11.8 lifecycle** — 🟡 **CLV implementado** (2026-06-07): [closing-line-capture-soccer.js](server/closing-line-capture-soccer.js) — análogo league-aware del MLB; lee la apertura del snapshot `odds_details` del pick, captura el cierre en vivo cerca del kick-off, `computeSoccerClv` = implied cierre − implied apertura; persiste `closing_odds`/`implied_prob_closing`/`clv`. Job cada 2h gated por `SOCCER_ANALYSIS_ENABLED`. `extractSoccerPickOdds`/`impliedProbPct`/`computeSoccerClv` puros. Tests: `soccerClosingLine.test.js` (6). Pendiente: postmortem sport-aware; ensemble de mercados soccer (requiere picks resueltos).
