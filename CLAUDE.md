...
- [server/services/calibrationBlockService.js](server/services/calibrationBlockService.js) — (2026-06-11) cierra el loop calibración→Oracle: `buildCalibrationBlock(sport)` renderiza el bloque `ORACLE CALIBRATION RECORD` (confianza declarada vs win rate real por mercado+bucket, ≥15 picks resueltos, cache 2h, flags OVERCONFIDENT/underconfident) con su propia `ORACLE INSTRUCTION` de anclaje. Apendeado al contexto de los 6 deportes: `context-builder.js` (MLB, patrón Lessons, sin tocar el prompt frozen) y `analyze*Game` de `oracleNba/Nfl/Nhl/Soccer/Tennis` (solo game analysis, no chat). Never-throws, `''` sin muestra suficiente, sin flag. Tests: `calibrationBlock.test.js` (5).
...
- **Pendiente operacional**: enriquecimiento in-season real (validación E2E del parlay/board, calibración de thresholds del Imperdible NFL, entrenamiento real del modelo `nfl_prop`) — todo requiere temporada (kickoff sept 2026, sin data viva en off-season).

### NHL (🟢 Sprint 10 — backend + UI + ML sidecar 10e completos)
...
- **Ya en código**:
  ...
  - [server/services/nhlShadowValidator.js](server/services/nhlShadowValidator.js) + [nhlShadowPersistence.js](server/services/nhlShadowPersistence.js) — validador determinístico (goal diff/special teams/goalie/form/rest, cap 70) + persistencia `pick_features`/`shadow_model_runs` `sport='nhl'`.
  - **ML sidecar 10e (2026-06-11)**: [ml/hexa_ml/nhl_history_loader.py](ml/hexa_ml/nhl_history_loader.py) — pre-training histórico desde la **API oficial NHL** (`api-web.nhle.com/v1/club-schedule-season/{TEAM}/{season}`, gratis sin key; ~32 requests/temporada, dedupe por game id, defensivo never-fatal): frame as-of sin leakage (goal diff/GF-GA per game/points% con punto de OT-SO/last10/rest/B2B snapshoteados ANTES de cada juego). Pre-entrena `nhl_moneyline` y **`nhl_puckline`** (la línea ±1.5 es FIJA en NHL → labels market-aligned sin odds históricas); `nhl_total` es live-only a propósito (línea variable, la fuente no trae mercados — entrenar a línea fija fingiría sensibilidad a la línea). [models/nhl.py](ml/hexa_ml/models/nhl.py) (reg fuerte), `features.py` `NHL_BASE_NUMERIC`+`add_nhl_derived` (de-vig 2-vías, special_teams_edge, points_pct_diff), `data.py` sport `nhl` + columnas + filtros + targets (puckline reusa la matemática runline >1.5), `train.py` `NHL_MARKETS`/`NHL_PRETRAIN_MARKETS`, `serve.py` `POST /predict/nhl_moneyline|nhl_puckline|nhl_total` + retrain. Flags `NHL_PRETRAIN_ENABLED` (default true) + `NHL_PRETRAIN_SEASONS`. Node: [server/services/nhlMlClient.js](server/services/nhlMlClient.js) — circuit breaker propio, `buildNhlFeaturePayload` (espeja el featureSnapshot de la persistencia shadow), `predictNhlGameModel` (3 mercados en paralelo). Tests: `test_nhl_history_loader.py` (8 py) + `nhlMlClient.test.js` (3 node). **Pendiente op**: deploy sidecar + `/retrain all` → verificar `nhl_*` en `/health`; cablear `predictNhlGameModel` en analyze/parlay para `python_pick_prob` (in-season).
...
- **Pendiente**: `hexaNhlBoardService` (pizarra), live tracker NHL, wiring de `predictNhlGameModel` en el flujo analyze (python_pick_prob → conviction/Bet Card), validación E2E en prod. La tab Live/Standings/Parlay muestran "fase posterior" hasta entonces.
