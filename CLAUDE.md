...
### NFL (✅ Sprint 9 + 9.4 + 9.5 completo — tercer deporte activo)
...
**NFL — ✅ Sprint 9 + 9.3 + props/board/parlay (9f–9j) + 9.4 + 9.5 completos (2026-06-07)**: tercer deporte operativo. Spec en [docs/nfl-architecture.md](docs/nfl-architecture.md), roadmap en [docs/nfl-roadmap.md](docs/nfl-roadmap.md). Todo el código está mergeado: Oracle, lifecycle, shadow/dataset, live tracker, UI, ML scaffolding + nflverse pre-training (9a–9e + 9.3), **player props** (9f fundación+board → 9g modelo `nfl_prop` pooled → 9h enriquecimiento player-level nflverse + persistencia `source='live'`), **pizarra `hexaNflBoardService` + UI de props parametrizada por sport** (9i), **Parlay Synergy NFL MVP** vía el motor frozen (9j, flag `PARLAY_SYNERGY_NFL_ENABLED`), y **precision parity con MLB** (9.4): 6 métricas nflverse nuevas (RZ TD%, 3rd-down, sack rate), superficie/altitud de estadio, fatiga acumulada, QB suplente, shadow validator con `situationalAdvantage`+`trenchesAdvantage`, y framework de coherencia de 8 señales en el prompt. Pendiente: **Imperdible NFL** (diferido — el más acoplado a MLB, necesita gate QB + data de temporada); entrenamiento real del modelo `nfl_prop` + enriquecimiento in-season de model probs en el parlay + validación E2E requieren temporada (sept 2026, sin data viva en off-season).
