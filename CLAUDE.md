...
### NFL (✅ Sprint 9 + 9.4 completo — tercer deporte activo)
...
- **Pendiente operacional**: **Imperdible NFL** (diferido a propósito — es el más acoplado a MLB: `MARKET_VARIANCE`/thresholds y el gate "lineup confirmado" asumen béisbol; necesita gate "QB confirmado" + key numbers + 4-6 semanas de data de parlay para calibrar la convicción); enriquecimiento in-season de model probs en el parlay NFL (vía `nfl_moneyline`/`spread`/`total`); `NFL_LIVE_TRACKER_ENABLED`. Entrenamiento real del modelo `nfl_prop` + validación E2E del parlay/board requieren temporada (sept 2026).
...
- `NFL_ANALYSIS_ENABLED` — habilita endpoints Oracle NFL + resolver NFL (default `false`; activo en Railway con `=true`). Opcionales planificados: `NFL_LIVE_TRACKER_ENABLED`, `IMPERDIBLE_NFL_ENABLED`. Ver [docs/nfl-roadmap.md](docs/nfl-roadmap.md).
