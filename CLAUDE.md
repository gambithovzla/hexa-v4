...
- **Completo (props 9f — fundación + board, 2026-06-06)**: Player Props NFL Fase 1, espeja el patrón MLB props (board ML + chat extraction; **no toca** el prompt Oracle de juego, que sigue bloqueando props). Cero ediciones a frozen.
  ...
  - [server/routes/nfl.js](server/routes/nfl.js) — `GET /api/nfl/props/board` (admin-only, flag `NFL_PROPS_ENABLED`): games + prop odds + fairProb + `oraclePropPicks` del usuario (picks `sport='nfl'` clasificados con `parseNflProp`). `modelProb=null` hasta el modelo dedicado.
...
- **Pendiente operacional**: `hexaNflBoardService.js` (sept 2026); **NFL prop ML model** (columnas `pick_features` + modelos `nfl_prop_*` pooled + enriquecimiento player-level nflverse) — Fase 2; Parlay/Imperdible multi-sport (Fase 3); `NFL_LIVE_TRACKER_ENABLED`. Validación E2E del board/resolver props requiere temporada (sept 2026, sin data viva en off-season).
...
- `NFL_PROPS_ENABLED` — habilita el board de player props NFL (`GET /api/nfl/props/board`, admin-only; default `false`). El resolver de props (boxscore ESPN) corre siempre dentro del resolver NFL, sin gate. `modelProb` queda null hasta el modelo ML dedicado.
