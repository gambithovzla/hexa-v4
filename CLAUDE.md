...
- **Parity con MLB (planificado, Sprint 11.2–11.9 — ver [docs/roadmap.md](docs/roadmap.md))**: Soccer pasó el MVP pero le faltan los pilares de profundidad que hicieron robusto a MLB (Sprint 8d context enrichment + props + ensemble + pretraining). Brechas rojas vs release-gate: **data depth pregame (4.5)**, **lineups/injuries/suspensiones (2.0)**, **market coverage/props (5.5)**, **calibración ML (3.0)**. Mismo patrón espejo de NFL 9.x, cero ediciones a frozen:
  ...
  - **11.3 profundidad de contexto pregame** (el "Statcast del fútbol"): el context-builder hoy solo deriva de standings + form-string `WDLWW`. Falta: **lineups + injuries + suspensiones** (API-Football, freemium; `lineupStatus:'unknown'` está hardcodeado; las suspensiones por amarillas/roja son únicas del fútbol y hoy invisibles); **árbitro** (tendencia tarjetas/penaltis, análogo umpire MLB); **congestión de calendario/rotación** (UCL/UEL + copa entre semana, análogo `buildScheduleFatigueBlock`); **weather** (el builder dice explícitamente "NO weather block" — pero el fútbol es outdoor; Open-Meteo ya está en MLB); splits local/visitante; H2H; métricas FBref (PPDA, pases progresivos, xG de balón parado); xG rolling 7d/14d + splits; motivación/stakes (descenso, título, plazas europeas, dead rubber).
