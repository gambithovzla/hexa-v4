...
- **Parity con MLB (planificado, Sprint 11.2–11.9 — ver [docs/roadmap.md](docs/roadmap.md))**: Soccer pasó el MVP pero le faltan los pilares de profundidad que hicieron robusto a MLB (Sprint 8d context enrichment + props + ensemble + pretraining). Brechas rojas vs release-gate: **data depth pregame (4.5)**, **lineups/injuries/suspensiones (2.0)**, **market coverage/props (5.5)**, **calibración ML (3.0)**. Mismo patrón espejo de NFL 9.x, cero ediciones a frozen:
  ...
  - **11.5 player props** (espeja NFL 9f–9i): cero props hoy. Tiros a puerta, goles, asistencias, tarjetas, pases, faltas. `soccer-props-resolver.js` + `soccer-props-odds.js` + `soccerPropFeatureEnricher.js` + `GET /api/soccer/props/board` + modelo `soccer_prop` pooled + UI. Flag `SOCCER_PROPS_ENABLED`.
