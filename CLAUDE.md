...
- **Pendiente operacional**: **Fase 2.1** — fetcher de stats player-level nflverse (season/recent avg, análogo a `nfl-advanced-fetcher.js`) para poblar `nfl_prop_player_*` y una ruta de persistencia `source='live'` de las features del prop al crearse el pick (hoy el chat persiste `source='oracle_chat'`, excluido del training). Sin eso el modelo se apoya solo en señal de mercado. `hexaNflBoardService.js` (sept 2026); Parlay/Imperdible multi-sport (Fase 3); `NFL_LIVE_TRACKER_ENABLED`. Entrenamiento real + validación E2E requieren temporada (sept 2026).
