...
- `NFL_PROPS_ENABLED` — habilita el board de player props NFL (`GET /api/nfl/props/board`, admin-only; default `false`) y el mercado `prop` en el chat extractor NFL. El resolver de props (boxscore ESPN) corre siempre dentro del resolver NFL, sin gate. El modelo `nfl_prop` (pooled) adjunta `modelProb` cuando el sidecar está entrenado.
