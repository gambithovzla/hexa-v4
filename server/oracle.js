...
1. Switch to a different market/side where your model has a positive edge, or
2. Keep the pick but set bet_value to "NO VALUE", set kelly_recommendation to "RECOMENDACIÓN KELLY: Sin ventaja matemática — No apostar." (ES) / "KELLY RECOMMENDATION: No mathematical edge — Do not bet." (EN), and add "Negative edge — informational only, no real value" to alert_flags.
...
function parseResponse(raw) {
  ...
  function sanitize(obj) {
    ...
    if (obj?.probability_model?.note !== undefined) {
      ...
      next = { ...obj, probability_model: rest };
    }
    // Strip fabricated American odds from best_pick.detail:
    // Valid American odds are ≥ +100 or ≤ -100. Anything in (-99..+99) is invalid and
    // comes from Oracle hallucination when the real line isn't in MARKET ODDS.
    const detail = next?.best_pick?.detail;
    if (typeof detail === 'string') {
      const cleaned = detail.replace(/\s*\(([+-]\d{1,2})\)/g, '').replace(/\s+/g, ' ').trim();
      if (cleaned !== detail) {
        console.warn('[oracle] stripped invalid fabricated odds from best_pick.detail:', detail, '->', cleaned);
        next = { ...next, best_pick: { ...next.best_pick, detail: cleaned } };
      }
    }
    return next;
  }
...
These additions make Premium feel substantially different from Deep. The user should read the Premium report and immediately see they got more depth, more options, and more actionable intelligence.`;
  ...
  if (mode === 'single' && data && !parseError) {
    try {
      ...
      const disagree = oracleWinnerId !== validatorWinnerId;

      if (disagree) {
        console.log(
          `[oracle] Validator divergence detected — Oracle: ${oracleWinnerId} vs Validator: ${validatorWinnerId}. ` +
        ...
        if (Array.isArray(data.alert_flags)) {
          data.alert_flags.push(
            `Validador estadístico diverge del Oracle — riesgo elevado a alto`
          );
        }
      }
...
export async function analyzeChat({ contextString, question, conversationHistory = [], lang = 'en' }) {
  const modelConfig = MODELS.deep; // Chat shares the Deep model (Opus 4.7)
...
5. Weather + park factors (NOTE: temperature is contextual only — never use it to generate thermal cross-game patterns or directional picks)
