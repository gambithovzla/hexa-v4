...
export async function analyzeSafe({ contextString, lang = 'en' }) {
  ...
  const userMessage = lang === 'es'
    ? `Analiza este partido para el motor Safe Pick objetivo. Resume el matchup, el riesgo y las notas por familia de mercado soportada (Moneyline, Run Line, Over/Under). No elijas el pick final: el backend lo rankea de forma determinista.\n\nDatos:\n${contextString}`
    : `Analyze this game for the objective Safe Pick engine. Summarize the matchup, risk, and notes by supported market family (Moneyline, Run Line, Over/Under). Do not choose the final pick: the backend ranks it deterministically.\n\nData:\n${contextString}`;
