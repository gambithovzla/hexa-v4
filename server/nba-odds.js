/**
 * nba-odds.js — NBA market odds via The Odds API (basketball_nba).
 *
 * Isolated from the MLB odds-api.js (frozen by convention) so we never risk
 * regressing MLB pricing. Mirrors the same key handling (primary + backup) and
 * normalisation strategy: top-3 books, consensus implied probability, American
 * format.
 *
 * Public API:
 *   getNbaGameOdds({ date })  → cached array of normalized NBA events
 *   matchNbaOddsToGame(events, homeTeamName, awayTeamName)
 *   buildMarketOddsForGame(event)  → { moneyline, spread, total } the Oracle expects
 */

const ODDS_API_BASE = 'https://api.the-odds-api.com/v4';
const SPORT_KEY     = 'basketball_nba';
const CACHE_TTL_MS  = 5 * 60 * 1000; // 5 min — NBA odds tighten fast pre-tip
...
function getDateWindow(date) {
  ...
  // 36h window: a calendar day at America/New_York can spill into the next UTC day
...
export function getNbaOddsStatus() {
  return {
    ..._lastFetchMeta,
    keyConfigured: Boolean(process.env.ODDS_API_KEY),
    backupKeyConfigured: Boolean(process.env.ODDS_API_BACKUP_KEY),
...
/**
 * Public: cached NBA market odds for a calendar date.
 * Never throws — on failure returns `[]` and exposes the error via getNbaOddsStatus().
 */
export async function getNbaGameOdds({ date } = {}) {
  ...
  const tryKey = async (apiKey, slot) => {
    try {
      ...
      console.log(`[nba-odds] ${SPORT_KEY} ${date ?? 'upcoming'}: ${data.length} events (key=${slot})`);
      return { slot, data };
    } catch (err) {
  ...
  if (primary) {
    ...
    if (backup && backup !== primary) {
      ...
      if (isOutOfCredits || r.error) {
        console.warn(`[nba-odds] primary key failed (${isOutOfCredits ? 'OUT_OF_USAGE_CREDITS' : r.error?.message ?? r.result?.status}) — trying backup`);
    ...
    setLastFetchMeta({
      ...
      status: r.result?.status ?? 'fetch_error',
      ok: false,
  ...
  setLastFetchMeta({
    ...
    requestedDate: date,
    events: cached?.data?.length ?? 0,
    status: rb.result?.status ?? 'fetch_error',
    ok: false,
...
function normalizeEvent(event) {
        ...
        case 'spreads':
          for (const o of market.outcomes ?? []) {
            if (o.name === event.home_team) {
              spHome.push(o.point); spHomePrice.push(o.price);
            } else {
              spAway.push(o.point); spAwayPrice.push(o.price);
            }
          }
          break;
        case 'totals':
  ...
  if (mlH == null && mlA == null) return null;
  ...
  return {
    ...
    spread: {
      home: spHome.length ? +(avg(spHome).toFixed(1)) : null,
      homePrice: consensusAmerican(spHomePrice),
      away: spAway.length ? +(avg(spAway).toFixed(1)) : null,
      awayPrice: consensusAmerican(spAwayPrice),
    },
    total: {
      line:       totals.length ? +(avg(totals).toFixed(1)) : null,
...
/**
 * Fuzzy match an NBA event by team names. Accepts full names (e.g. "Boston Celtics")
 * or short names ("Celtics") — the word-overlap heuristic copes with both.
 */
export function matchNbaOddsToGame(events, homeTeamName, awayTeamName) {
...
/**
 * Convert a matched NBA event into the `marketOdds` shape the Oracle prompt expects:
 * `{ moneyline: { home, away, homeImplied, awayImplied }, spread, total }`.
