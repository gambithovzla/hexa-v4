/**
 * nhl-odds.js — NHL market odds via The Odds API (icehockey_nhl).
 *
 * Isolated from the frozen MLB odds-api.js (and the NBA/NFL modules) so we never
 * regress other sports. Mirrors the dual-key handling and normalisation of
 * nfl-odds.js, with the NHL market shape: MONEYLINE is primary, the PUCK LINE is
 * a fixed ±1.5 handicap (taken by MODE, not averaged, so we never manufacture a
 * fake 1.0 or 2.0), and the TOTAL is goals (also by mode). Prices use consensus.
 *
 * Public API:
 *   getNhlGameOdds({ date })  → cached array of normalized NHL events
 *   matchNhlOddsToGame(events, homeTeamName, awayTeamName)
 *   buildMarketOddsForGame(event)  → { moneyline, puckLine, total }
 *   getNhlOddsStatus()
 */
...
function getDateWindow(date) {
  ...
  // NHL plays daily; a 2-day window from the requested date covers timezone slop.
...
export function getNhlOddsStatus() {
  return {
    ..._lastFetchMeta,
    keyConfigured: Boolean(process.env.ODDS_API_KEY),
    backupKeyConfigured: Boolean(process.env.ODDS_API_BACKUP_KEY),
...
/**
 * Public: cached NHL market odds for a calendar date. Never throws — on failure
 * returns `[]` and exposes the error via getNhlOddsStatus().
 */
export async function getNhlGameOdds({ date } = {}) {
  ...
  const tryKey = async (apiKey, slot) => {
    try {
      ...
      console.log(`[nhl-odds] ${SPORT_KEY} ${date ?? 'upcoming'}: ${data.length} events (key=${slot})`);
      return { slot, data };
    } catch (err) {
  ...
  if (primary) {
    ...
    if (backup && backup !== primary) {
      ...
      if (isOutOfCredits || r.error) {
        console.warn(`[nhl-odds] primary key failed (${isOutOfCredits ? 'OUT_OF_USAGE_CREDITS' : r.error?.message ?? r.result?.status}) — trying backup`);
...
/** Most common value (preserves the real ±1.5 puck line); first on a tie. */
function mode(arr) {
  if (!arr.length) return null;
...
function normalizeEvent(event) {
  ...
  const mlHome = [], mlAway = [];
  const plHome = [], plHomePrice = [], plAway = [], plAwayPrice = [];
  ...
  for (const book of books) {
    for (const market of book.markets ?? []) {
      switch (market.key) {
        ...
        case 'spreads':
          for (const o of market.outcomes ?? []) {
            if (o.name === event.home_team) { plHome.push(o.point); plHomePrice.push(o.price); }
            else { plAway.push(o.point); plAwayPrice.push(o.price); }
          }
          break;
        case 'totals':
          for (const o of market.outcomes ?? []) {
  ...
  const plH = plHome.length ? mode(plHome) : null;
  const plA = plAway.length ? mode(plAway) : null;
  const totLine = totals.length ? mode(totals) : null;
  if (mlH == null && mlA == null && plH == null && totLine == null) return null;
  ...
  return {
    ...
    puckLine: {
      home: plH,
      homePrice: consensusAmerican(plHomePrice),
      away: plA,
      awayPrice: consensusAmerican(plAwayPrice),
    },
    total: {
      line:       totLine,
...
/**
 * Fuzzy match an NHL event by team names. Accepts full names ("Toronto Maple
 * Leafs") or short names ("Maple Leafs") — word-overlap copes with both.
 */
export function matchNhlOddsToGame(events, homeTeamName, awayTeamName) {
...
/**
 * Convert a matched NHL event into the `marketOdds` shape the Oracle prompt
 * expects: `{ moneyline, puckLine, total }` (moneyline first — NHL primary
 * market). Returns null if the event has no usable price.
 */
export function buildMarketOddsForGame(event) {
  ...
  return {
    ...
    puckLine: {
      home:      event.puckLine.home,
      homePrice: event.puckLine.homePrice,
      away:      event.puckLine.away,
      awayPrice: event.puckLine.awayPrice,
    },
    total: {
