/**
 * server/services/imperdibleArbiter.js — LLM risk auditor for Pick Imperdible.
 *
 * Owns its own Anthropic client (per project convention; does NOT import
 * oracle.js). Receives the pre-ranked deterministic lock candidates + full game
 * context and returns a CONFIRM/PASS verdict. Defaults to Opus for maximum
 * reasoning quality on the single most important bet of the day.
 */

import Anthropic from '@anthropic-ai/sdk';
import { IMPERDIBLE_ARBITER_PROMPT, buildImperdibleArbiterUserMessage } from '../prompts/imperdible-prompts.js';
 ...
 * @returns {Promise<{verdict, selected_candidate_id, ...}>}
 */
export async function arbitrateImperdible({ candidates, gameContexts, lang = 'en', timeoutMs = 60000 }) {
  ...
  const userMessage = buildImperdibleArbiterUserMessage({
    lang,
  ...
  const message = await anthropic.messages.create(
    {
      ...
      system: [
        { type: 'text', text: IMPERDIBLE_ARBITER_PROMPT, cache_control: { type: 'ephemeral' } },
      ],
  ...
  if (!normalized) {
    console.warn('[imperdible-arbiter] failed to parse verdict, defaulting to PASS');
    return {
      verdict: 'PASS',
      selected_candidate_id: null,
      confidence: null,
