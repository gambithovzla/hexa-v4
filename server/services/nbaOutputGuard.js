import { NBA_OUTPUT_SCHEMA_VERSION } from '../prompts/oracle-nba-prompts.js';

const ALLOWED_BET_TYPES = new Set(['moneyline', 'spread', 'total', 'overunder', 'ou']);
...
export function validateNbaAnalysisOutput(data, { parseError = false } = {}) {
  if (parseError) {
    return {
      ok: false,
      quality: 'reject',
      errors: ['json_parse_failed'],
      schema_version: NBA_OUTPUT_SCHEMA_VERSION,
      data: null,
    };
  }
  ...
  if (data.parlay) {
    return {
      ok: false,
      quality: 'reject',
      errors: ['parlay_shape_on_single_game'],
      schema_version: NBA_OUTPUT_SCHEMA_VERSION,
      data: null,
    };
  }
  ...
  else if (conf < 50 || conf > 78) errors.push('confidence_out_of_range');
