...
export async function computeAdminEquity({ sport = 'all', startDate, endDate } = {}) {
  ...
  for (const row of rows) {
    ...
    const dd = runningUnits - peakUnits; // 0 or negative
  ...
  // Per-pick Sharpe (mean / stddev of flat-bet returns)
  let sharpe = 0;
  if (perPickReturns.length >= 2) {
    const mean     = perPickReturns.reduce((a, b) => a + b, 0) / perPickReturns.length;
    const variance = perPickReturns.reduce((a, b) => a + (b - mean) ** 2, 0) / (perPickReturns.length - 1);
