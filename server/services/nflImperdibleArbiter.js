/**
 * server/services/nflImperdibleArbiter.js — LLM risk auditor for NFL Pick
 * Imperdible. Mirrors imperdibleArbiter.js (MLB) with the NFL prompt. Owns its
 * own Anthropic client (project convention; does NOT import oracle.js). Defaults
 * to Opus for the single most important NFL bet of the slate.
 */

import Anthropic from '@anthropic-ai/sdk';
import {
  NFL_IMPERDIBLE_ARBITER_PROMPT,
  buildNflImperdibleArbiterUserMessage,
} from '../prompts/nfl-imperdible-prompts.js';
 ...
 */
export async function arbitrateNflImperdible({ candidates, gameContexts, lang = 'en', timeoutMs = 60000 }) {
  ...
  const userMessage = buildNflImperdibleArbiterUserMessage({
    lang,
  ...
  const message = await anthropic.messages.create(
    {
      ...
      system: [
        { type: 'text', text: NFL_IMPERDIBLE_ARBITER_PROMPT, cache_control: { type: 'ephemeral' } },
      ],
  ...
  if (!normalized) {
    console.warn('[nfl-imperdible-arbiter] failed to parse verdict, defaulting to PASS');
    return {
      verdict: 'PASS',
      selected_candidate_id: null,
      confidence: null,
